...
int main()
{
	...
	while(1) 
	{
		...
		switch(num)
		{
			case 1:inputmedicine(head);break;/*输入药品信息*/
			case 2:printmedicine(head);break;/*打印药品信息报表*/
			case 3:findmedicine(head);break;/*查询对应药品信息*/
			case 4:modisymedicine(head);break;/*修改药品信息*/
			case 5:delatemedicine(head);break;/*删除药品信息*/
			case 6:sortmedicine(head);break;/*给药品排序*/
			case 7:insertmedicine(head);break;/*在随意位置插入药品信息*/
			case 8:countmedicine(head);break;/*简单统计药品数量*/
			case 9:statisticsmedicine(head);break;/*药品数据综合统计*/
			case 10:
				printf("欢迎下次使用~\n");
				return;
			default:
...
int login_user()
{
    ...
    struct user temp;
    while (fread(&temp, sizeof(struct user), 1, file) == 1) 
	{
...
void delate_user()
{
    ...
    while (fread(&temp, sizeof(struct user), 1, file) == 1) {
        ...
        if (strcmp(username, temp.username) == 0 && strcmp(password, temp.password) == 0) {
            ...
            fwrite(&temp, sizeof(struct user), 1, temp_file);
        }
    }
...
void keepmedicine(struct node*head)
{
    ...
    while(move!=NULL)
    {
        if(fwrite(&move->mc,sizeof(struct medicine),1,file)!=1)
        {
            printf("保存出现错误\n");
        }
        move=move->next;
...
{
    FILE* file = fopen("./gs.info", "r");/*打开对应文件*/ 
    if (!file) {
        printf("没有药品文件,跳过读取\n");
        return;
    }
    ...
    while (fread(&temp, sizeof(struct medicine), 1, file) == 1)
    {
        struct node* fresh = malloc(sizeof(struct node));
        fresh->mc = temp;
        fresh->next = NULL;
